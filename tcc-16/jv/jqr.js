$(document).ready(function() {
    ...
    // Enviar comentário quando o formulário for enviado
    $('#commentForm').submit(function(e) {
        e.preventDefault();
        var userName = $('#userName').val();
        var commentContent = $('#commentContent').val();

        $.post('adicionar_comentario.php', {
            userName: userName,
            ...
            $('#userName').val('');
            $('#commentContent').val('');
            loadComments(); // Recarregar comentários após o envio
        });
    });

    // Função para carregar comentários
    function loadComments() {
        $('#commentList').load('carregar_comentarios.php');
    }
});
